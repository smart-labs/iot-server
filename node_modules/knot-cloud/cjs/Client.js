...
var _meshblu = require('@cesarbr/meshblu');

var _meshblu2 = _interopRequireDefault(_meshblu);
...
function createConnection(hostname, port, uuid, token) {
  return _meshblu2.default.createConnection({
    server: hostname,
    port,
    uuid,
    token
...
function mapData(data) {
  const newData = _lodash2.default.omit(data, ['uuid', 'source', '_id']);
  newData.data = _lodash2.default.omit(newData.data, ['uuid', 'token']);
  return newData;
}

function mapDevice(device) {
  return _lodash2.default.omit(device, ['uuid', '_id', 'owner', 'type', 'ipAddress', 'token', 'meshblu', 'discoverWhitelist', 'configureWhitelist', 'socketid', 'secure', 'get_data', 'set_data']);
}
...
function parseSetDataInput(data) {
  if (!_lodash2.default.isArrayLikeObject(data)) {
  ...
  for (let i = 0; i < data.length; i += 1) {
    const item = data[i];
    if (item.sensorId === null || item.sensorId === undefined || item.value === null || item.value === undefined) {
...
function parseRequestDataInput(sensors) {
  if (!_lodash2.default.isArrayLikeObject(sensors)) {
    throw new Error('sensors must be an array');
  }

  return _lodash2.default.map(sensors, sensorId => ({ sensor_id: sensorId }));
}
...
class Client {
    ...
    return new Promise((resolve, reject) => {
      _request2.default.get({
        ...
        if (err || res.statusCode !== 200) {
          ...
          resolve(_lodash2.default.map(body.data, mapData));
        }
      });
    });
  }
  ...
  on(callback) {
    ...
    this.connection.on('message', data => {
      if (callback && data.payload && _lodash2.default.indexOf(this.subscriptions, data.payload.source) !== -1) {
        callback(_lodash2.default.set(mapData(data.payload), 'source', this.uuidIdMap[data.payload.source]));
      }
    });
  }
}

exports.Client = Client; // eslint-disable-line import/prefer-default-export